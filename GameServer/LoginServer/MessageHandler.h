...
public:
	...
	MessageHandler(TcpDataHandler* _tcpDataHndler,int _tcpFd, string _clientIP,int _clientPort);
	~MessageHandler();
   ...
   void SendMessage(GameSingleTLV* _singleTLV, int _tcpFd);
private:
	int m_tcpFD;
	string m_clientIp;
	int m_clientPort;
	std::list<GameSingleTLV*> m_MsgList;
	TcpDataHandler* m_tcpDataHndler;
};
