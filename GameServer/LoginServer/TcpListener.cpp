...
bool TcpListener::handler()
{
	...
	if (0 <= iDataFd)
	{
		...
		bRet = GameServer::GetInstance().Add_Channel(*_tcpDataHandler);
...
bool TcpDataHandler::handler()
{
	...
	//std::cout << "<----------------------------------------->" << std::endl;
	//std::cout << "recv from " << m_fd << ":" << GameServer::Convert2Printable(recvStr) << std::endl;
	//std::cout << "<----------------------------------------->" << std::endl;
	...
	if (false == bRet)
	{
		...
		string _loginServer = LOGINSERVER_IP + to_string(LOGINSERVER_PORT);
		string _client = m_clientIP + to_string(m_clientPort);
		if(_client != _loginServer)
		{
			GameServer::GetInstance().UpdateGameServerInfo2LoginServer(-1);
			GameServer::GetInstance().PlayerLogout(m_playerName);
		}
		SetChannelClose();
		//GameServer::GetInstance().GetListener()->DeleteServerFromList(this->GetFd());
	}
	/*
	...
	while (m_lastBuf.size() >= 8)
	{
		...
		*/
		...
		if (m_lastBuf.size() - 8 >= len)
		{
			...
			{
				msgHandler = new MessageHandler(this,this->GetFd(),m_clientIP,m_clientPort);
			}
			//将singletlv加到GameMsg
